...
TEST(StkIo_legacy, sideset_polarity_AB)
{
  stk::ParallelMachine pm = MPI_COMM_WORLD;
...
TEST(StkIo_legacy, check_internal_sideset_warning_with_reconstruction)
{
    stk::ParallelMachine pm = MPI_COMM_WORLD;
...
TEST(StkIo_legacy, check_internal_sideset_warning_with_incremental_update)
{
    stk::ParallelMachine pm = MPI_COMM_WORLD;
    if (stk::parallel_machine_size(pm) != 1) {
      return;
